module Exercise.Chapter02 where
{-
...
findK k l = if k == 1 then head l else findK (k-1) (tail l)

-- Determine if list l is a palindrome
isPalindrome l = length l < 2 || head l == last l && (isPalindrome . init . tail) l
...
{-
 ...
 -}
ziplike xs ys = if null xs || null ys then []
...
splitAtIndex k l = if k == 0 then ([], l)
  else let 
    (left, right) = splitAtIndex (k - 1) (tail l)
  in (head l: left, right)

-- Drop the element at index k in list l
-- For example "dropK 3 [0,0,0,1,0,0,0]" returns [0,0,0,0,0,0]
dropK k l = if k == 0 then tail l
